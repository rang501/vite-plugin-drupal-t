...
items\`, {});`;
    ...
    describe('filter options', () => {
        it('should respect include option', () => {
            plugin = extractDrupalT({ include: '**/*.js' });
            const code = `const msg = Drupal.t('Test');`;
            plugin.transform?.call(createPluginContext(), code, 'test.js');
            const output = generateOutput(plugin);
            expect(output).toContain(`// Drupal.t('Test')`);
            plugin = extractDrupalT({ include: '**/*.js' });
            plugin.transform?.call(createPluginContext(), code, 'test.ts');
            const output2 = generateOutput(plugin);
            expect(output2).toBe('');
        });
        it('should respect exclude option', () => {
            plugin = extractDrupalT({ exclude: '**/*.test.js' });
            const code = `const msg = Drupal.t('Test');`;
            plugin.transform?.call(createPluginContext(), code, 'main.js');
            const output = generateOutput(plugin);
            expect(output).toContain(`// Drupal.t('Test')`);
            plugin = extractDrupalT({ exclude: '**/*.test.js' });
            plugin.transform?.call(createPluginContext(), code, 'file.test.js');
            const output2 = generateOutput(plugin);
            expect(output2).toBe('');
        });
    });
...
function generateOutput(plugin) {
    const context = createPluginContext();
    ...
    if (typeof generateBundle === 'function') {
        ...
        generateBundle.handler.call(context, {}, {}, false);
    }
